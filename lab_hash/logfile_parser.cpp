...
LogfileParser::LogfileParser( const string & fname ) : whenVisitedTable( 256 ) {
    ...
    while( infile.good() ) {
         ...
         * vector as well.
         */
    string haha = ll.url+ll.customer;   
    //update the date of visited
    if (whenVisitedTable.keyExists(haha)){
        if(whenVisitedTable.find(haha) < ll.date)
            whenVisitedTable[haha] = ll.date;
    }
    else{
        whenVisitedTable[haha]=ll.date;
        }
   //if the url is not in the page visited
...
//    (void) customer; // prevent warnings... When you implement this function, remove this line.
  //  (void) url;      // prevent warnings... When you implement this function, remove this line.
    ...
    return (whenVisitedTable.keyExists(url+customer)); // replaceme
}
...
time_t LogfileParser::dateVisited( const string & customer, const string & url ) const {
  ...
  //  (void) customer; // prevent warnings... When you implement this function, remove this line.
    ...
    if (hasVisited(customer,url))
        return whenVisitedTable.find(url+customer);
    else
