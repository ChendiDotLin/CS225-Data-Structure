...
int main()
{
	...
	cout<<set->getpicture(2)<<endl;
	result.writeToFile("scene.png");	

	set->changemaxlayers(5);
	result = set->drawscene();
	cout<<set->getpicture(2)<<endl;
	result.writeToFile("scenetest.png");	
	
	set->changemaxlayers(2);
        result = set->drawscene();
        //cout<<set->maxnum<<endl;
        result.writeToFile("scenetest4.png");
	set->changemaxlayers(4);
        result = set->drawscene();
        cout<<set->getpicture(1)<<endl;
        result.writeToFile("scenetest1.png");

	set->changemaxlayers(9);
        result = set->drawscene();
        cout<<set->getpicture(0)<<endl;
        result.writeToFile("scenetest2.png");
