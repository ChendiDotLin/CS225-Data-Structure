...
//	PronounceDict b(a);
	string worda = word1;
	std::transform(worda.begin(),worda.end(),worda.begin(), ::toupper);
	string wordb = word2;
	std::transform(wordb.begin(),wordb.end(),wordb.begin(), ::toupper);
	auto lookup1 = dict.find(worda);
	auto lookup2 = dict.find(wordb);
	if (lookup1!=dict.end() && lookup2!=dict.end()){
	bool check = true;
	for (size_t i =0 ; i<lookup1->second.size(); i++){
		check = check && (lookup1->second[i] == lookup2->second[i]);
		}
	//check = lookup1->second == lookup2->second;
		return check;
	}
	else 
	return false;
