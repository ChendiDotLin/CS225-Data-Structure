...
class BinaryTree
{
	...
	std::vector<T> isOrdered(const Node * subroot, std::vector<T> &myvec) const;
      // bool isOrdered(const Node * subroot) const; 
    	...
    	void printPaths(Node * subroot,std::vector<T> myvec) const;  
        /**
	...
	int sumDistances(Node *subroot, int i , int &result) const;    
    private:
