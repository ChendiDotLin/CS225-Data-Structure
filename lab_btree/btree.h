...
size_t idxhelper(const std::vector< T >& elements, const C& val, size_t left, size_t right)
{
	if (left==right) {
		if (val > elements[left]) return left+1;
		else return left;
		}
	size_t mid = (left+right)/2;
	if (val == elements[mid]) return mid;
	if (val < elements[mid]) return idxhelper(elements,val, left,mid);
	else return idxhelper(elements, val, mid+1 , right);
}
...
size_t insertion_idx(const std::vector< T >& elements, const C& val)
{
   ...
   return idxhelper(elements, val, 0, elements.size()-1);
}
