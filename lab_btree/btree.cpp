...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
	...
	//parent->children.insert(child_itr,new_right);
	//parent->elem_itr.insert(elem_itr,
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if (subroot->is_leaf) {
	auto it1 = subroot->elements.begin() + first_larger_idx;
	auto it2 = subroot->children.begin() + first_larger_idx;
	subroot->elements.insert(it1, pair);
	subroot->children.insert(it2, NULL);
	return;
	}		
