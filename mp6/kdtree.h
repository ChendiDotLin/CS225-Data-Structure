...
class KDTree
{
    ...
    private:
         ...
         */
         void treebuilder(vector< Point<Dim> > & points, int dim, int left, int right);
         void swap(Point<Dim> & a , Point<Dim> & b );
         int partition (int dimension, vector< Point<Dim> > & list, int left, int right, int pivotIndex);
         Point<Dim> qselect(int dimension, vector<Point <Dim> > & list, int left, int right, int n );
	 Point<Dim> finder(const Point<Dim> & target, Point<Dim>  haha, int left, int right, int dim)const;
	int radius(const Point<Dim> & first, const Point<Dim> & second) const;
	int distance(const Point<Dim> & first, const Point<Dim> & second, int dim) const;
};
