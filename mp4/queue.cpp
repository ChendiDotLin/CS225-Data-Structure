...
T Queue<T>::dequeue()
{
     ...
     */
	if (outStack.isEmpty()){ //so that we cannot just pop out the first element in outstack
		while (!inStack.isEmpty()){
		T temp = inStack.remove();
		outStack.add(temp);
		}
		}
...
T Queue<T>::peek()
{
     ...
     */
 	if (outStack.isEmpty()){
	   ...
	   return outStack.peek();//the first element in outStack should always be the first to be added in 
}
