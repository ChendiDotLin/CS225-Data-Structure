...
{	
        ...
        {
	int r = original(xi,yi)->red;
	int g = original(xi,yi)->green;
	int b = original(xi,yi)->blue;
	if (r+amount > 255) {
	original(xi,yi)->red = 255;}
	else { original(xi,yi)->red =r+amount;}
	if (g+amount > 255) {                                                   
        original(xi,yi)->green = 255;}       
	 else { original(xi,yi)->green =g+amount;}
	 if (b+amount > 255) {                                                  
        original(xi,yi)->blue= 255;}    
	 else { original(xi,yi)->blue=b+amount;}			
            /// Your code here!
        }
    }
    return original;
