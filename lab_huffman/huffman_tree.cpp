...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	...
	TreeNode * a = new TreeNode(frequencies.at(i));
	singleQueue.push(a);
		}
	TreeNode * a = NULL;
	TreeNode * b = NULL;
...
//		TreeNode * a ;
//		TreeNode * b ;
		a = removeSmallest(singleQueue, mergeQueue);
		b = removeSmallest(singleQueue, mergeQueue);
		sum = new TreeNode (a->freq.getFrequency() + b->freq.getFrequency());	
		sum->left = a;
		sum->right = b;
		mergeQueue.push(sum);
	}
	 	this->root = removeSmallest(singleQueue, mergeQueue); 
	//	delete a;
	//	delete b;	
	//	delete sum;
}
...
HuffmanTree::TreeNode * HuffmanTree::readTree( BinaryFileReader & bfile ) {
	...
	while (bfile.hasBits()){
		if(bfile.getNextBit()) {
			Frequency* b = new Frequency (bfile.getNextByte(),0);
